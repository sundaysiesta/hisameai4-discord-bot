...
const createLeaderboardEmbed = async (users, page, totalPages, title, type) => {
    ...
    const descriptionPromises = currentUsers.map(async (u, i) => {
        ...
        return `**${start + i + 1}位:** <@${u.userId}> - Lv.${level} (${u.xp} XP)`;
    });
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('top')
        .setDescription('レベルランキングを表示します。')
        .addStringOption(option => option.setName('type').setDescription('ランキングの種類（未指定で両方表示）').setRequired(false).addChoices({ name: 'テキスト', value: 'text' }, { name: 'ボイス', value: 'voice' }))
