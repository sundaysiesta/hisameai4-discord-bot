...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('clearxpdata')
        .setDescription('破損した可能性のある月間・日間のボイスXPデータをクリアします。(管理者限定・一度だけ使用)'),
    async execute(interaction, redis) {
        ...
        try {
            ...
            const allKeys = [...monthlyVoiceKeys, ...dailyVoiceKeys];
            if (allKeys.length > 0) {
                await redis.del(allKeys);
                await interaction.editReply(`破損した可能性のあるボイスXPデータキーを ${allKeys.length} 件削除しました。これ以降、XPは正常に記録されます。`);
            } else {
